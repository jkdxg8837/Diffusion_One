...
for gamma in stable_gamma_list:
    ...
    plt.plot(ckpt_steps, step_wise_lower_stage_means, label=f'Lower Stage', color='blue', marker='o')
    plt.plot(ckpt_steps, step_wise_medium_stage_means, label=f'Medium Stage', color='orange', marker='o')
    plt.plot(ckpt_steps, step_wise_higher_stage_means, label=f'Higher Stage', color='green', marker='o')

    plt.xlabel('Step')
    plt.ylabel('Mean Frobenius Norm')
    plt.title(f'Stage-wise Mean Frobenius Norms Across Steps for baseline')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    plt.savefig(f"{path}/baseline.png")
    plt.close()
pass
